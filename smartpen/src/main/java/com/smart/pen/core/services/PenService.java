...
public abstract class PenService extends Service {
	...
	public SceneType getSceneType() {
		...
		SceneType type = SceneType
				.toSceneType(preferences.getInt(Keys.DEFAULT_SCENE_KEY, SceneType.NOTHING.getValue()));
		...
		if (pointList != null && pointList.size() > 0) {
			for (int i = 0; i < pointList.size(); i++) {
				...
				if (mScenePointObject.getSceneType() == SceneType.CUSTOM) {
					item.setCustomScene(mScenePointObject.getWidth(), mScenePointObject.getHeight(),
							mScenePointObject.getOffsetX(), mScenePointObject.getOffsetY());
				} else {
				...
				// 定位完第一个坐标，笔被抬起后，记录状态
				if (mFirstPointDown && !item.isRoute)
