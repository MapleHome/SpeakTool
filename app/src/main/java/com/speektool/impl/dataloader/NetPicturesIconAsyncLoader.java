...
import com.speektool.busevents.NetPictureThumbnailLoadedEvent;
...
public class NetPicturesIconAsyncLoader extends
	...
	protected PicDataHolder getDataLogic(String key, Object... args) {
		...
		try {
			...
			if (isGif(key)) {
				...
				data.gif = UniversalHttp.downloadFile(key,
						"http://img4.duitang.com");
				if (data.gif == null)
					return null;
				...
				PicDataHolder data = new PicDataHolder();
				byte[] bytesBig = UniversalHttp.downloadFile(key,
						"http://img4.duitang.com");// mem p.
	...
	@Override
	protected void postFinishEvent(String key, PicDataHolder value,
			Object... args) {
		...
		if (BitmapScaleUtil.isGif(key)) {
			...
			Bitmap bpScaled = BitmapFactory.decodeByteArray(value.bpScaled, 0,
					value.bpScaled.length);
			d = new BitmapDrawable(bpScaled);

		}
		if (d != null)
			EventBus.getDefault().post(
					new NetPictureThumbnailLoadedEvent(key, d, false));
		else
	...
	@Override
	protected void postErrorEvent(String key, PicDataHolder value,
			Object... args) {
		EventBus.getDefault().post(
				new NetPictureThumbnailLoadedEvent(key, null, true));
	}
