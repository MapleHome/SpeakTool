...
import com.speektool.tasks.TaskUploadException;
import com.speektool.tasks.TaskUploadException.UploadExceptionCallback;
...
public class ErrorService extends Service {
	...
	private static void savaRecentException(final Throwable ex, String expFileName) {
		...
		try {
			...
			e.printStackTrace();
			try {
				File tempf = new File(SpeekToolApp.app().getApplicationInfo().dataDir, expFileName);
				if (!tempf.exists())
					tempf.createNewFile();
				PrintWriter pw = new PrintWriter(tempf);
				ex.printStackTrace(pw);
