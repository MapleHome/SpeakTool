...
import com.ishare_lib.ui.dialog.AlertEditTextDialog;
import com.ishare_lib.ui.dialog.AlertEditTextDialog.EditInputTextCallback;
...
public class UserInfoChangePage extends BaseFragment implements OnClickListener {
	...
	private void changePortrait(View anchor) {
		new ActionSheetDialog(mActivity).builder()
				...
				}).addSheetItem("本地相册", SheetItemColor.Blue, new OnSheetItemClickListener() {
					@Override
					public void onClick(int which) {
	...
	private void changeName() {
		new AlertEditTextDialog(mActivity).builder().setTitle("请输入昵称").setMsg("起一个好听的名字，让更多人认识你！")
		// .setHint("请不要超过10个字符")
				.setPositiveButton(new EditInputTextCallback() {
					@Override
	...
	private void changeIntroduce() {
		new AlertEditTextDialog(mActivity).builder().setTitle("请输入个性签名").setPositiveButton(new EditInputTextCallback() {
			@Override
			public void getInputText(String text) {
				if (!TextUtils.isEmpty(text)) {
	...
	private void changeMail() {
		new AlertEditTextDialog(mActivity).builder().setTitle("绑定新邮箱").setPositiveButton(new EditInputTextCallback() {
			@Override
			public void getInputText(String text) {
				if (!TextUtils.isEmpty(text)) {
					tv_mail.setText(text);
				}
			}
		}).show();
	}
