...
import com.speaktool.impl.modes.StrokeHandler;
import com.speaktool.impl.paint.DrawPaint;
import com.speaktool.ui.dialogs.LoadingDialogHelper;
import com.speaktool.utils.DisplayUtil;
import com.speaktool.utils.ScreenFitUtil;
import com.speaktool.utils.T;
import com.speaktool.utils.UiHandler;
			...
			public void onActivityResult(int requestCode, int resultCode, Intent data) {
				if (resultCode == Activity.RESULT_OK) {
					if (requestCode == REQUEST_SETTING_SIZE) {
						PenService service = SpeakToolApp.app().getPenService();
...
//		draw.getIBISPenController().destroy();
		...
		case CONNECT_TYPE_BLUETOOTH:// 蓝牙
			showDialog();
			SpeakToolApp.app().unBindPenService();
			SpeakToolApp.app().bindPenService(Keys.APP_PEN_SERVICE_NAME);
			break;
		case CONNECT_TYPE_USB:// USB
			showDialog();
			SpeakToolApp.app().unBindPenService();
			SpeakToolApp.app().bindPenService(Keys.APP_USB_SERVICE_NAME);
			break;
		default:
			return;
		}
	...
	private static class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			if (msg.what == MSG_CHECK_BIND_SERVICE) {
				if (SpeakToolApp.app().getPenService() == null) {
	...
	private void initSceneType() {
		final PenService service = SpeakToolApp.app().getPenService();
