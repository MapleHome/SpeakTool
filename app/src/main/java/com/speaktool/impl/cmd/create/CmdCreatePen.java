...
public class CmdCreatePen extends CmdCreateShape<CreatePenData> {
	...
	public void run(final Draw draw, Page bw) {
		...
		long preT = 0;
		final Path_ mPath_ = new Path_(new Path(), data.getShapeID(), color,
		...
		for (int i = 0; i < size; i++) {
			...
			if (j == 0) {
				...
				draw.postTaskToUiThread(new Runnable() {// UP.
					...
					public void run() {
						draw.getCurrentBoard().drawOnTemp(null);// must set to
																// null.
						boolean isMoved = mv.getX() - points.get(0).getX() != 0
								|| mv.getY() - points.get(0).getY() != 0;
						if (isMoved) {
							draw.getCurrentBoard().drawOnBuffer(mPath_);
							draw.getCurrentBoard().saveShape(mPath_);
						} else {
							Point_ point = new Point_(mv.getX(), mv.getY(),
									mPath_.getId(), color, strokeWidth,
									isEraser);
				...
				draw.postTaskToUiThread(new Runnable() {// move.
					@Override
					public void run() {
						mPath_.getPath().quadTo(preMvCopy.getX(),
								preMvCopy.getY(),
