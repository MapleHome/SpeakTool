...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
	 ...
	 * @param resolver
	 *            resolver used to query {@code uri}, cannot be null
	 * @throws IOException
	 ...
	 * calls have no effect. Like {@link Bitmap#recycle()} this
	...
	public String toString() {
		return String.format(Locale.US, "GIF: size: %dx%d, frames: %d, error: %d", mNativeInfoHandle.width,
				mNativeInfoHandle.height, mNativeInfoHandle.imageCount, mNativeInfoHandle.getNativeErrorCode());
	}
	 ...
	 * large. Method is thread-safe. Decoding is performed in background thread
	 * and drawable is invalidated automatically afterwards.
	 * 
	 * @param position
	...
	/**
	 * Like {@link GifInfoHandle#seekToTime(long, int, Bitmap)}
	...
	/**
	 * Sets whether this drawable is visible. When the drawable becomes
	 * invisible, it will pause its animation. A subsequent change to visible
	 * with <code>restart</code> set to true will restart the animation from the
	 ...
	 * @param restart
	 *            when visible, true to force the animation to restart from the
	 *            first frame
